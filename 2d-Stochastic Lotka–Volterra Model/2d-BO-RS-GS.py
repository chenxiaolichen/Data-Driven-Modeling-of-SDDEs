...
if __name__ == "__main__":
    ...
    if trajectories is not None:
        print("\n--- Starting Efficiency Comparison (2D System) ---")

        # Grid Search
        print("Running Grid Search...")
        start_gs = time.time()
        taus_gs = np.linspace(SEARCH_SPACE[0][0], SEARCH_SPACE[0][1], N_SCAN_STEPS_GS)
        mis_gs = [-objective_function_mi_negative([tau]) for tau in taus_gs]
        duration_gs = time.time() - start_gs
        best_idx_gs = np.argmax(mis_gs)
        best_tau_gs = taus_gs[best_idx_gs]
        summary_list.append({'Method': 'Grid Search (GS)', 'Identified τ': best_tau_gs,
                             'Runtime (s)': duration_gs, 'Evaluations': N_SCAN_STEPS_GS})
        ...
        result_bo = gp_minimize(
            ...
            random_state=41, verbose=False, noise="gaussian"
        )
        ...
        summary_list.append({'Method': 'Bayesian Optimization (BO)', 'Identified τ': best_tau_bo,
                             'Runtime (s)': duration_bo, 'Evaluations': N_CALLS})
        ...
        summary_list.append({'Method': 'Random Search (RS)', 'Identified τ': best_tau_rs,
                             'Runtime (s)': duration_rs, 'Evaluations': N_CALLS})

        # Save comparison table
        summary_df = pd.DataFrame(summary_list)
        print("\n==================== Quantitative Performance Comparison (2D System) ====================")
        ...
        csv_path = os.path.join(OUTPUT_DIR, "performance_comparison_table_2D.csv")
        summary_df.to_csv(csv_path, index=False)
        print(f"Table saved to: {csv_path}")

        # ==============================================================================
        # Advanced Visualization: Left Performance + Right Dual-Cost Bar Charts
        # ==============================================================================
        print("\nGenerating advanced dual-cost comparison plot...")
        ...
        ax_main.scatter(taus_rs, mis_rs, c=colors[1], s=100, alpha=0.7, edgecolors='darkgreen', marker='s',
                        label='Random Search (RS)')
        ax_main.scatter(taus_bo, mis_bo, c=colors[2], s=100, alpha=0.8, edgecolors='navy',
                        label='Bayesian Optimization (BO)')
        ...
        ax_main.legend(fontsize=19.5, loc='best', frameon=False)
        ax_main.spines['right'].set_visible(False)

        # Show results
        ax_main.text(0.95, 0.95,
                     f'GS Identified τ = {best_tau_gs:.4f}\n\nRS Identified τ = {best_tau_rs:.4f}\n\nBO Identified τ = {best_tau_bo:.4f}',
                     transform=ax_main.transAxes, fontsize=19, verticalalignment='top', horizontalalignment='right',
        ...
        ax_time.set_title('Runtime', fontsize=20)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, "figure_2_efficiency_comparison_2D_advanced.png"),
