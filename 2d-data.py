        ...
        for j in range(1, num_steps + 1):
            if t_values[j] <= 0:
                ...
                drift_x, diffusion_x, drift_y, diffusion_y = sdde_adapter(
                    t_values[j], [x_values[j - 1], x_delay], [y_values[j - 1], y_delay],
                    tau)
...
def visualize_trajectories(x_matrix, y_matrix, t_values, n_samples=500, title="SDDE轨迹", save_path=None):
    ...
    for i in indices:
        plt.plot(t_values, x_matrix[i, :, 0], alpha=0.7, label='X轨迹')
    ...
    for i in indices:
        plt.plot(t_values, y_matrix[i, :, 0], alpha=0.7, label='Y轨迹')
...
T = 10.0  # 终止时间
h = 0.01  # 时间步长
n_dimensions = 1  # 状态维度（这里虽然写1，但实际处理二维数据）
n_traj = 500  # 轨迹数量
seed = 42  # 随机种子
output_file = r"\2dSDDE\2d_sdde_data.npz"
...
def sample_data(sdde, tau, T, h, n_dimensions, n_traj, alpha_values, seed=None):
    ...
    return (
        ...
        np.array(all_x_next),
        np.array(all_y_next),
        x_matrix,  # (n_alpha, n_traj, n_time, dim)
        y_matrix,  # (n_alpha, n_traj, n_time, dim)
        t_values[start_index:],
        alpha_values
    )
...
def visualize_trajectories(x_matrix, y_matrix, t_values, alpha_values, n_samples=50, save_path=None):
    ...
    for a_idx, alpha in enumerate(alpha_values):
        ...
        ax1.set_title(f'alpha={alpha} - X轨迹', fontsize=15)
        ...
        ax2.set_title(f'alpha={alpha} - Y轨迹', fontsize=15)
    ...
    plt.show()
# ====================== 配置参数 ======================
tau = 1.0  # 初始延迟时间（后续贝叶斯优化会识别）
T = 10.0  # 终止时间
h = 0.01  # 时间步长
n_dimensions = 1  # 每个状态维度（整体为2D系统）
n_traj = 200  # 每个参数的轨迹数量
seed = 42  # 随机种子
alpha_values = np.array([1.3, 1.5, 1.6, 1.8])  # 多参数集合
output_file = r"E:\2D-BO-MI\2d多参数\2d_multi_param_sdde_data.npz"
plot_samples = 50  # 每个参数可视化的轨迹数
plot_file = r"E:2D-BO-MI\2d多参数\trajectory_plot.png"
